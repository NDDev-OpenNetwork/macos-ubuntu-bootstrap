...
The contexts `main` enforces are stated in two places, and both must agree:
...
`scripts/ci/check_required_contexts.py` reads the mirror on every test run and
compares it with the branch when it is given `--live` and can reach the API.
A third list used to sit here — the control plane's own copy in
`.gds/repository.yaml` — and it was bound by nothing: it happened to be
accurate, and nothing would have noticed if it stopped being. It is gone with
the anchor, so the mirror and the branch are the only two, which is one fewer
...
3. Then, in one act: add it to the live ruleset and to this mirror.

```bash
# 1. read the live ruleset and confirm the mirror still matches it everywhere else
gh api /repos/NDDev-OpenNetwork/macos-ubuntu-bootstrap/rules/branches/main \
  --jq '[.[] | select(.type=="required_status_checks")
         | .parameters.required_status_checks[].context] | sort'

# 2. apply the mirror
# The ruleset id is this repository's own; read it first rather than pasting one.
ruleset_id="$(gh api /repos/NDDev-OpenNetwork/macos-ubuntu-bootstrap/rulesets \
  --jq '.[] | select(.target=="branch") | .id' | head -1)"
gh api --method PUT "/repos/NDDev-OpenNetwork/macos-ubuntu-bootstrap/rulesets/${ruleset_id}" \
  --input .github/rulesets/branch-main.json

# 3. prove the mirror and the branch agree
